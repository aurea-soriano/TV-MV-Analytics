...
public class DistanceMatrix implements Cloneable {
    ...
    */
     ...
     * @param maxDistance
     * @params maxDistance The new maximum distance.
     */
     ...
     * @param minDistance
     * @params minDistance The new minimum distance.
     */
     ...
     * @param n
     * @params n The new element count of the distance matrix.
     */
    ...
    public void save(String filename) throws IOException {
        ...
        try {
            ...
            for (float[] distmatrix1 : this.distmatrix) {
                for (int j = 0; j < distmatrix1.length; j++) {
                    out.write(Float.toString(distmatrix1[j]));
        ...
        try {
            ...
            while (tUrls.hasMoreTokens()) {
                ...
                if (utils.Util.isParsableToInt(id)) {
                    ...
                    this.ids.add(utils.Util.convertToInt(id));
                }
            ...
            ArrayList<Float> cdata_aux = new ArrayList<>();
    ...
    public void removeElement(int id) {
        ...
        if (index != -1) {
            ...
            for (int row=0;row<index-1;row++) {
                System.arraycopy(distmatrix[row], 0, ndistmatrix[row], 0, distmatrix[row].length);
            ...
            for (int row=index;row<distmatrix.length;row++) {
                System.arraycopy(distmatrix[row], 0, ndistmatrix[row-1], 0, index);
                for (int col=index+1;col<distmatrix[row].length;col++)
                    ndistmatrix[row-1][col-1] = distmatrix[row][col];
            }
            ...
            System.arraycopy(cdata, 0, ncdata, 0, index);
            ...
            for (float[] distmatrix1 : distmatrix) {
                for (int col = 0; col < distmatrix1.length; col++) {
                    if (distmatrix1[col] < minDistance) {
                        minDistance = (float) distmatrix1[col];
                    }
                    if (distmatrix1[col] > maxDistance) {
                        maxDistance = (float) distmatrix1[col];
                    }
                }
            }
        }
