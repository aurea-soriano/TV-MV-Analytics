...
import projection.IDMAPProjection.InitializationType;

/**
 *
 * @author Fernando Vieira Paulovich
 */
public class ProjMeansProjection implements Projection {

    public ProjMeansProjection() {
    ...
    public AbstractMatrix project(AbstractMatrix matrix, AbstractDissimilarity diss) throws IOException {
        ...
        ArrayList<AbstractMatrix> projections = new ArrayList<>();
        ...
        for (ArrayList<Integer> cluster : clusters) {
            ...
            DistanceMatrix dmat_aux = this.createDistanceMatrix(matrix, cluster, diss);
    ...
    public AbstractMatrix project(DistanceMatrix dmat) throws IOException {
        ...
        for (ArrayList<Integer> cluster : clusters) {
            //Create a distance matrix
            DistanceMatrix dmat_c = this.createDistanceMatrix(dmat, cluster);
            IDMAPProjection idmap = new IDMAPProjection();
            idmap.setInitialization(ini);
            idmap.setFractionDelta(fracdelta);
            idmap.setNumberIterations(nriterations);
    ...
    private DistanceMatrix createDistanceMatrix(AbstractMatrix matrix,
        ...
        for (Integer cluster1 : cluster) {
            nMatrix.addRow(matrix.getRow(cluster1));
        }
    ...
    public void setCentroids(AbstractMatrix centroids) {
