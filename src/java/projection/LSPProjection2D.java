...
import clustering.neighbors.ANN;
import clustering.neighbors.KNN;
...
public class LSPProjection2D implements Projection {
    ...
    public AbstractMatrix project(AbstractMatrix matrix, AbstractDissimilarity diss) throws IOException {
        ...
        if (controlpoints == null || cpprojection == null) {
            controlpoints = getControlPoints(bkmeans, clusters, matrix);
            ...
            cpprojection = projectControlPoints(dmat_cp);
        }
        ...
        AbstractMatrix projection = createFinalProjection(mesh, matrix);
    ...
    protected AbstractMatrix projectControlPoints(DistanceMatrix dmat_cp) throws IOException {
        ...
        return cpprojection = idmap.project(dmat_cp);
    }
...
//        }
    ...
    private AbstractMatrix createFinalProjection(Pair[][] neighbors, DistanceMatrix dmat) throws IOException {
        AbstractMatrix projection = new DenseMatrix();
...
//            }
            ...
            for (Pair neighbor : neighbors[i]) {
                if (max < neighbor.value) {
                    max = neighbor.value;
                }
            ...
            for (Pair neighbor : neighbors[i]) {
                if (max > min) {
                    float dist = (((neighbor.value - min) / (max - min)) * (0.9f)) + 0.1f;
                    sum += (1 / dist);
                }
            }
            ...
            for (Pair neighbor : neighbors[i]) {
                if (max > min) {
                    float dist = (((neighbor.value - min) / (max - min)) * (0.9f)) + 0.1f;
                    A.setQuick(i, neighbor.index, -((1 / dist) / sum));
                } else {
                    A.setQuick(i, neighbor.index, -(1.0f / neighbors[i].length));
                }
            }
        }
        ...
        for (int i = 0; i < cpprojection.getRowCount(); i++) {
            B.setQuick((neighbors.length + i), 0, cpprojection.getRow(i).getValue(0));
            B.setQuick((neighbors.length + i), 1, cpprojection.getRow(i).getValue(1));
        }
    ...
    private void projectUsingProgram(Pair[][] neighbors, AbstractMatrix projection) throws IOException {
        long start = System.currentTimeMillis();
...
//                }
            ...
            for (Pair neighbor : neighbors[i]) {
                if (max > min) {
                    float dist = (((neighbor.value - min) / (max - min)) * (0.9f)) + 0.1f;
                    solver.addToA(i, neighbor.index, -((1 / dist) / sum));
                } else {
                    solver.addToA(i, neighbor.index, -(1.0f / neighbors[i].length));
                }
            }
        }
        for (int i = 0; i < nrcontrolpoints; i++) {
        ...
        for (int i = 0; i < cpprojection.getRowCount(); i++) {
            solver.addToB((neighbors.length + i), 0, cpprojection.getRow(i).getValue(0));
            solver.addToB((neighbors.length + i), 1, cpprojection.getRow(i).getValue(1));
        }
    ...
    private int[] getControlPoints(BKmeans bkmeans,
        ...
        if (type == ControlPointsType.KMEANS) {
            ...
            //medoids
            ArrayList<Integer> medoids_aux = new ArrayList<>();
            ...
            for (int i = 0; i < localcp.length; i++) {
                localcp[i] = medoids_aux.get(i);
            }
        } else if (type == ControlPointsType.RANDOM) {
            //Random choice
            localcp = new int[nrcontrolpoints];
            for (int i = 0; i < localcp.length; i++) {
                localcp[i] = (int) (Math.random() * matrix.getRowCount());
            }
        }
    ...
    public static void main(String args[]) throws IOException {
         String filename = "/home/aurea/MEGA/dados/diabetes.data";
         ...
         AbstractMatrix projection = lsp.project(matrix, new EuclideanSimilarity());
         long finish = System.currentTimeMillis();
