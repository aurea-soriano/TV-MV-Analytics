 ...
 */
public class FastmapProjection implements Projection {
    ...
    public AbstractMatrix project(DistanceMatrix dmat) throws IOException {
        try {
            ...
            float[][] points = points = new float[dmat.getElementCount()][];
            ...
            for (int i = 0; i < points.length; i++) {
                AbstractVector vector = new DenseVector(points[i], ids.get(i), cdata[i]);
                
                if (labels.isEmpty()) {
                    matrix.addRow(vector);
                } else {
                    matrix.addRow(vector, labels.get(i));
                }
            }
    ...
    private void doTheFastmapLessThan4Points(float[][] points, DistanceMatrix dmat) {
        if (points.length == 1) {
            points[0][0] = 0;
            points[0][1] = 0;
