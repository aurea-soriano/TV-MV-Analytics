...
public class BKmeans extends Clustering {
    ...
    public ArrayList<ArrayList<Integer>> execute(AbstractDissimilarity diss, AbstractMatrix matrix) throws IOException {
        try {
            ...
            ArrayList<Integer> gCluster = new ArrayList<>();
            ...
            for (int i = this.clusters.size() - 1; i >= 0; i--) {
                if (this.clusters.get(i).size() <= 0) {
                    ...
                    Logger.getLogger(this.getClass().getName()).log(Level.WARNING,
                            "The Bissection k-means algorithm is returning an " + "empty cluster. Number of requested clusters : {0}", this.nrclusters);
                }
            }
            ...
            if (removed > 0) {
                Logger.getLogger(this.getClass().getName()).log(Level.WARNING,
                        "The Bissection k-means algorithm is returning " + "empty clusters. Removed: {0}", removed);
            }

            long finish = System.currentTimeMillis();

            Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Bisecting K-Means time: {0}s", (finish - start) / 1000.0f);
