...
public class BKmedoids extends Clustering {
    ...
    private void splitCluster(int cluster, ArrayList<ArrayList<Integer>> clusters, DistanceMatrix dmat) {
        ...
        //For each cluster
        for (Integer oldCluster1 : oldCluster) {
            float distanceCentroid1 = dmat.getDistance(oldCluster1, medoids1);
            float distanceCentroid2 = dmat.getDistance(oldCluster1, medoids2);
            if (distanceCentroid1 < distanceCentroid2) {
                cluster1.add(oldCluster1);
            } else if (distanceCentroid1 > distanceCentroid2) {
                cluster2.add(oldCluster1);
            } else {
