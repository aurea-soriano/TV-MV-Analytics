...
public class ANN {
    ...
    public Pair[][] execute(AbstractMatrix matrix, AbstractDissimilarity diss,
            ArrayList<ArrayList<Integer>> clusters, AbstractMatrix centroids) throws IOException {
        ...
        for (int c = 0; c < clusters.size(); c++) {
            ...
            for (int e = 0; e < clusters.get(c).size(); e++) {
                ...
                for (Pair clustersNeighbor : clustersNeighbors[c]) {
                    count += clusters.get(clustersNeighbor.index).size();
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Approximated KNN time: {0}s", (finish - start) / 1000.0f);
