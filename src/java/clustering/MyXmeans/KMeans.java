...
public class KMeans extends PartitionClustering<double[]> implements Serializable {
    ...
    public int predict(double[] x) {
        ...
        for (int i = 0; i < k; i++) {
            double dist = squaredDistance(x, centroids[i]);
            if (dist < minDist) {
                minDist = dist;
                bestCluster = i;
            }
    ...
    public KMeans(double[][] data, int k, int maxIter, int runs) {
        ...
        try {
            ...
            System.out.println("Failed to run K-Means on multi-core");
     ...
     * contain missing values (i.e. Double.NaN). The algorithm runs up to 100
     * iterations.
     *
     * @param data the input data of which each row is a sample.
     * @param k the number of clusters.
