 ...
 * @author aurea
 */
public class UniformScaling extends Object implements AbstractDissimilarity {
    ...
    public float calculate(AbstractVector v1, AbstractVector v2) {
        ...
        for (int p = Q.length; p <= C.length; p++) {
            ...
            float distance = squaredEuclidean(QP, Ctrunk);
    ...
    float squaredEuclidean(float[] QP, float Ctrunk[]) {
        float sum = 0.f;
        for (int i = 0; i < QP.length && i < Ctrunk.length; i++) {
            sum += Math.pow(QP[i] - Ctrunk[i], 2);
        }
        return sum;
    }
